...
1. **Data Management**
   (`src/stress_prediction/data_management/task_data_management_template.py`)

   - Loads raw CSV data, cleans it, saves as pickle to `bld/data/`

1. **Analysis** (`src/stress_prediction/analysis/task_analysis_template.py`)

   - Fits logit models and generates predictions
   - Iterates over `TEMPLATE_GROUPS` defined in `config.py`

1. **Final Outputs** (`src/stress_prediction/final/task_final_template.py`)
...
- `src/stress_prediction/config.py`: Central path definitions (`SRC`, `ROOT`, `BLD`,
...
- `src/stress_prediction/`: Source code (hand-written)
